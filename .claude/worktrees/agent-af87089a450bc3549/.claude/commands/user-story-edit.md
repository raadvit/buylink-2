...
```bash
gh issue view {číslo} --repo radekvit/BuyLink
```
...
```bash
gh issue edit {číslo} --repo radekvit/BuyLink --add-label "status:{nový-status}"
```

Pokud je status `done`, uzavři issue:
```bash
gh issue close {číslo} --repo radekvit/BuyLink
```
