...
def submit():
    ...
    state_store.create_session(session_id, form_data)

    t = threading.Thread(
        target=story_builder.run_review,
        args=(session_id, form_data, state_store),
        daemon=True,
    )
    t.start()
...
def review_confirm():
    ...
    if not session:
        return jsonify({"error": "Session nenalezena."}), 404
    if not state_store.compare_and_set_status(session_id, "reviewing", "building"):
        return jsonify({"error": "Session není ve stavu reviewing."}), 409
    form_data = session["form_data"]
    review_comments = session.get("review_comments", [])
    thread = threading.Thread(
        target=story_builder.run_session,
        args=(session_id, form_data, state_store),
        kwargs={"extra_context": review_comments},
        daemon=True,
    )
    thread.start()
...
def review_comment():
    ...
    if not session:
        return jsonify({"error": "Session nenalezena."}), 404
    if not state_store.compare_and_set_status(session_id, "reviewing", "processing"):
        return jsonify({"error": "Session není ve stavu reviewing."}), 409
    comment_id = state_store.add_review_comment(session_id, comment)
    form_data = session["form_data"]
    thread = threading.Thread(
        target=story_builder.run_review_comment,
        args=(session_id, comment_id, comment, form_data, state_store),
        daemon=True,
    )
    thread.start()
    return jsonify({"ok": True})
...
def answer():
    ...
    found = state_store.set_answer(session_id, question_id, answer_text)
    if not found:
        return jsonify({"error": "Otázka s daným ID nenalezena."}), 404

    if state_store.all_questions_answered(session_id):
        state_store.update_session(session_id, status="building")
