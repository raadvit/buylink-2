...
async function submitBug() {
  ...
  if (!valid) {
    showStatus('error', 'Vyplň všechna povinná pole.');
    return;
  }

  window.scrollTo({ top: 0, behavior: 'smooth' });
  showStatus('loading', 'Vytvářím bug issue…');
  ...
  try {
    ...
    if (!res.ok) {
      showStatus('error', 'Chyba při vytváření issue.', json.error || '');
      return;
    }
    showStatus('success', 'Bug issue vytvořen: <a href="' + escHtml(json.issue_url) + '" target="_blank" rel="noopener">' + escHtml(json.issue_url) + '</a>');
    const done = document.getElementById('done-section');
    done.removeAttribute('hidden');
    const issueLabel = json.issue_number ? '#' + escHtml(String(json.issue_number)) + ' \u2014 zobrazit issue' : 'Zobrazit issue';
    document.getElementById('done-content').innerHTML =
      '<a href="' + escHtml(json.issue_url) + '" target="_blank" rel="noopener noreferrer">' + issueLabel + '</a>';
    document.querySelector('.btn-send').disabled = true;
  } catch (e) {
    showStatus('error', 'Síťová chyba.', e.message);
  }
}
...
function newBug() {
  ...
  document.getElementById('done-section').setAttribute('hidden', '');
