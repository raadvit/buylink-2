...
function _updateTimelineForPhase(phase, hasConflict) {
  ...
  tlWrap.hidden = false;
  const phaseMap = {
    'product_owner':    { status: 'draft',          activeStep: 'draft',           errorStep: null },
    'conflict_detector':{ status: 'conflict-check', activeStep: 'conflict-check',  errorStep: hasConflict ? 'conflict-check' : null },
    'architect':        { status: 'ready-for-arch', activeStep: 'ready-for-arch',  errorStep: null },
    'done':             { status: 'validated',       activeStep: null,              errorStep: null },
  };
  const cfg = phaseMap[phase];
  if (!cfg) return;
  renderTimeline(tlContainer, cfg.status, { activeStep: cfg.activeStep, errorStep: cfg.errorStep });
}
...
function startPolling() {
  ...
  _pollInterval = setInterval(async () => {
    if (Date.now() > _pollDeadline) {
      stopPolling();
      showStatus('error', 'Validace trvá příliš dlouho', 'Zkus to znovu nebo kontaktuj správce.');
      setFormDisabled(false);
      return;
    }
    try {
...
function handleSessionData(data) {
  ...
  if (status === 'asking_questions') {
    ...
    if (data.validation_phase && data.validation_phase !== 'done') {
      ...
      showProgressBar();
      showStatus('loading', data.review_summary || 'Agenti pracují…');
...
function renderDone(result, totalCostUsd) {
  ...
  _resetForm();
  _showDoneSection(result, totalCostUsd);
  window.scrollTo({ top: 0, behavior: 'smooth' });
}

function _showDoneSection(result, totalCostUsd) {
  ...
  if (typeof totalCostUsd === 'number' && totalCostUsd > 0) {
    html += '<span class="done-tokens">$' + totalCostUsd.toFixed(4) + '</span>';
  }
