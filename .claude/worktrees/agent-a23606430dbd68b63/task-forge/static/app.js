...
function _updateTimelineForPhase(phase, hasConflict) {
  ...
  tlWrap.hidden = false;
  const phaseMap = {
    'product_owner':    { status: 'draft',          activeStep: 'draft',          errorStep: null,                                    statusLabel: 'Product Owner zpracovává zadání',         statusState: 'working' },
    'conflict_detector':{ status: 'conflict-check', activeStep: 'conflict-check', errorStep: hasConflict ? 'conflict-check' : null,   statusLabel: hasConflict ? 'Konflikty detekováno' : 'Kontrola konfliktů…', statusState: hasConflict ? null : 'working' },
    'architect':        { status: 'ready-for-arch', activeStep: 'ready-for-arch', errorStep: null,                                    statusLabel: 'Architekt přidává technické anotace',    statusState: 'working' },
    'done':             { status: 'validated',       activeStep: null,             errorStep: null,                                    statusLabel: 'Validace dokončena',                      statusState: 'done' },
  };
  const cfg = phaseMap[phase];
  if (!cfg) return;
  renderTimeline(tlContainer, cfg.status, { activeStep: cfg.activeStep, errorStep: cfg.errorStep, statusLabel: cfg.statusLabel, statusState: cfg.statusState });
}
