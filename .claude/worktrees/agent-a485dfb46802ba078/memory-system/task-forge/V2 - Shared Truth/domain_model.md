...
## Timeline Widget — Tlačítko "Pustit implementaci"
...
| `ready_for_testing` / `ready-for-testing` | `in-development` |
...
|---|---|---|
| `.done` | zelená (`#43a047`) | `idx <= lastIdx` — krok byl již úspěšně dokončen; nebo `idx === activeIdx` (aktuální fáze se vykresluje jako passed) |
| `.current` | modrá (`#1976d2`, glow efekt) | `idx === activeIdx + 1` — krok hned za aktuální fází; nebo pokud `activeIdx === -1` pak `idx === lastIdx + 1` |
...
- `statusLabel` — vlastní text statusu
- `statusState` — stav banneru (`'working'`, `'done'`)
...
| (implementace spuštěna) | `in-development` | `in-development` | `null` |

### Tlačítko "Pustit implementaci" (US-041)

V banneru pod timelinerem se zobrazuje tlačítko umožňující admino spustit implementaci story.

**Zobrazení**: Tlačítko se renduje v `bannerHtml` (pravá část banneru) pouze když:
- Story status je `ready-for-arch` nebo `validated`
- (Ekvivalentně: modrý puntík timeline ukazuje na `arch-approved` (dev plán, idx 3) nebo se právě aktualizuje na `in-development` (vývoj, idx 4))

**Chování**:
1. Admin klikne "Pustit implementaci"
2. Tlačítko se **ihned disabluje** (disabled attribute, opacity 0.5, cursor not-allowed)
3. POST request `/api/implement` se pošle asynchronně (bez čekání)
4. Loading bar se zobrazí s textem "Programátoři pracují na implementaci"
5. GitHub issue se aktualizuje na status `in-development` (backend akce)
6. Timeline se aktualizuje dle pollingování — přechází na krok `in-development` (idx 4)
7. Tlačítko **zůstane trvale disabled**, i po refreshi stránky (pokud session pokračuje)

**API kontrakt**:
- **Endpoint**: POST `/api/implement`
- **Request**: `{ "story_id": <int>, "issue_number": <int> }`
- **Response (success)**: `{ "success": true, "session_id": "<uuid>" }` (HTTP 202)
- **Response (failure)**: `{ "error": "<message>" }` (HTTP 400/500)
- **Backend validace**:
  - Issue musí existovat a mít label "user story"
  - Story status musí být `ready-for-arch` nebo `validated`
  - GitHub API aktualizace musí projít; pokud selže, session se označí error a agent se nespustí
  - Agent `/implement` se spustí v background vlákně (daemon)

**Chybové stavy**:
- Pokud network call selže → inline chyba pod timelinerem (element `#impl-error-msg` s CSS třídou `.impl-error`), tlačítko zůstane disabled
- Pokud backend vrátí chybu (4xx/5xx) → zobrazit chybu, tlačítko zůstane disabled
- Backend musí validovat, že story je v správném stavu (bezpečnostní check, řádka 290 v task-forge.py)

**CSS styling**:
- Barva: zelená (`#43a047`), shodná s timeline `.done` stavem a submit button
- Font: stejný jako ostatní tlačítka v banneru
- Padding: stejný jako ostatní tlačítka
- Disabled stav: opacity `0.5`, cursor `not-allowed`, žádný hover efekt

**Frontend komponenty**:
- `renderTimeline()` v `shared.js` (řádky 46-85) — renderuje tlačítko v `bannerHtml` (řádka 78-79)
- `_bindImplButton()` v `app.js` (řádka 794) — navěšuje event listener
- `_onImplClick()` v `app.js` (řádka 805) — handler s POST /api/implement a error handling
- CSS třída `.btn-implementation` — stylování (HTML/CSS v task-forge/static/)

**Kdy se timeline aktualizuje po kliknutí**:
- Frontend polling (`/api/session`) detekuje `session.status === 'in_development'`
- `_updateTimelineForPhase('in-development', false)` se zavolá
- Timeline step index 4 (`in-development`) se změni z `.current` (modrý) na `.done` (zelený)
- Předchozí krok index 3 (`arch-approved`) zůstane `.done` (zelený)

### CSS transitions

`.story-step`, `.story-step::before`, `.story-step::after` mají `transition: .3s ease` na `color`, `background`, `box-shadow`.
...
- **Spuštění implementace** (US-041) - tlačítko "Pustit implementaci" je viditelné ve stavech `ready-for-arch` a `validated`; po kliknutí se timeline aktualizuje na `in-development`
...
Banner s tlačítkem se renderuje jako součást `renderTimeline()` v `bannerHtml`:

```html
<div class="story-status">
  <div class="story-status-icon"><!-- loading dots nebo checkmark --></div>
  <div class="story-status-text"><span class="who">Status label</span></div>
...
- **Co dělá**: Posuzuje business zadání — zda je story dostatečně popsána. Pokud chybí klíčové informace, může položit max. 3 otázky (omezeno polem `max_questions` v instrukčním souboru). Generuje: `why` (business goal), `what` (co se zobrazuje), `how` (jak se chová), `reads_sections`, `writes_sections`.
...
### POST `/api/submit`
Uloží story do GitHubu a spustí agentový pipeline.
- **Body (JSON)**: `{ name, epic, role, what, how, scope?, deps?, ac?, flags?, issue_number?, wiki_path? }`
- **Povinná pole**: `name`, `epic`, `role`, `what`, `how`
- **Odpověď (success)**: `{ session_id, issue_number, issue_url, wiki_path }` (HTTP 202)
- **Odpověď (failure)**: `{ error: "..." }` (HTTP 500) — pokud uložení do GitHubu selže, pipeline se nespustí
- **Efekt**: (1) Synchronně uloží draft do GitHub issue se statusem `conflict-check` přes `story_builder.build_draft_body()` — pokud existuje `issue_number`+`wiki_path`, aktualizuje; jinak vytvoří nový. (2) Vytvoří session s `issue_number` a `wiki_path`. (3) Spustí `run_team_validation()` asynchronně v novém vlákně.
- **Garantuje**: Story je vždy v GitHubu před spuštěním validace — data nelze ztratit při selhání agentů.
...
**REMOVED** — vrací HTTP 410 Gone. Story se vytváří automaticky po dokončení validace (přechod `building` → `done`). Manuální potvrzení není potřeba.
...
- Q: Měla by se změna aplikovat na všechny role nebo jen na admina?
  A: vše
- Q: Týkají se změny šířky POUZE sekcí 'why' a 'epic', nebo se změní layout i ostatních prvků na stránce?
  A: týká se to celého sloupce, tzn. všech widgetů v daném sloupci
- Q: Jaké budou konkrétní šířkové poměry v jednotlivých stavech? (např. 70/30 → 30/70, nebo jiný poměr?)
  A: tak jako nyní ale otočené
- Q: Zůstane 'wide right' layout na zbytku životního cyklu story (od validation do done), nebo se změní v jiných stavech?
  A: ve stavu PR a DONE (timeline) se šířka vrátí do původní podoby
- Q: Proč měníme 'Načíst myšlenku' z pop-up modálu na full stránku? Jaký je business přínos — chcete na tu stránku přidat více funkcí, nebo je to zlepšení UX?
  A: lepší UX
- Q: Odkud se spouští tlačítko 'Načíst myšlenku'? V jakém kontextu jej uživatel vidí — v detailu story, v přehledu, v headeru?
  A: z hlavičky
- Q: Header komponenta s tlačítky (nahlásit myšlenku, bug, požadavek, vytvořit myšlenku) — má být na všech stránkách fixně viditelná? Jak se má pozicovat (top bar, side panel)?
  A: ano na všech
- Q: Jaká je business motivace za tímto tlačítkem? Proč chcete explicitní tlačítko v UI místo backgroundního spuštění implementace? (Lepší descoberability pro adminy, audit trail, kontrola nad timingem?)
  A: chci vše ovládat z FE
- Q: Když admin klikne 'Pustit implementaci', co se konkrétně stane na backendu? Spustí se existující `/implement` agent, nebo je to nový proces? Změní se story status na `in_development` v tu chvíli?
  A: na back-end se volá /implement dané storky
- Q: Když bude tlačítko disabled po kliknutí — bude trvale disabled (dokud story neprojde do jiného stavu), nebo existuje nějak way jak ho znovu aktivovat, třeba pokud se implementace nezdaří?
  A: neexistuje
- Q: Co se má stát po kliknutí na 'Pustit implementaci' u story, která NENÍ ještě validovaná architektem? Má se zahájit arch review, nebo se development spustí bez validace?
  A: pustí se /implement, vždy se pustí /implement, orchestrace je tam
- Q: Je 'Dev plán' (arch-approved) a 'Vývoj' (in-development) dva nezávislé kroky timeline, nebo jsou to dva substáty jedné fáze implementace?
  A: nezávislé kroky viz agents.md
- approved - první krok vývoje (architekt)
- in development (druhý krok vývojáři)
- Q: Jaká je business motivace pro manuální tlačítko — proč se story automaticky nespustí na in-development hned po architektem validaci?
  A: pustí viz tvůj dotaz 1, problém není v tlačítku, ale storkou řeším stavy (barvy) v timelien
- Q: Jaká je business motivace změny barvy? (User feedback, design rebrand, accessibility požadavek, či jiné?)
  A: něco testuju pak to změním zpět
- Q: Je to izolovaná změna jen pro toto tlačítko, nebo součást širší design změny? (Jsou postižena další tlačítka/prvky?)
  A: něco testuju pak to změním zpět
- Q: Jaká konkrétní barva se má použít? (HEX kód, RGB, nebo máš referenci na design system?)
  A: něco testuju pak to změním zpět
- Q: Jaký je business důvod změny barvy na žlutou? (viditelnost tlačítka, upozornění na důležitý krok, vizuální redesign, nebo něco jiného?)
  A: jen něco testuju
- Q: Která barva je v tuto chvíli? (abych pochopil kontrastnost změny)
  A: jen něco testuju
- Q: Platí to pro všechny role, nebo jen pro roli 'admin'? Je 'admin' role uživatele, který tlačítko vidí, nebo role, která změnu vyžádala?
  A: jen něco testuju
<!-- /SECTION: project-qa -->
...
**CSS třídy**: `.btn-send` (nová story), `.btn-send-saved` (uložený draft)
**Barva pozadí**: `#fdd835` (žlutá)
**Barva textu**: `#212121` (tmavá — kontrast WCAG AA)
**Funkce**: Odesílá formulář k agentové validaci (spouští Product Owner review)
**Historie barev**: #43a047 zelená (US-033) → #e91e8c růžová (US-051) → #fdd835 žlutá (aktuální)
...
<!-- SECTION: story-ui-layout -->
## Story Form Layout — Responsive Grid

Formulář story se vykresluje v responzivní mřížce (`.r-main`), která se mění dle fáze validace:

### Stavy layoutu

| Fáze story | Grid template | Popis |
|---|---|---|
| `draft` | `1fr 280px` | Standardní: levý sloupec (formulář) zabírá flexibilní místo, pravý (agent) 280px |
| `conflict-check` | `1fr 280px` | Zůstává standardní — agenti pracují |
| `ready-for-arch` | `1fr 280px` | Zůstává standardní — čeká na architekturu |
| `arch-approved` (validace hotova) | `280px 1fr` | **Otočený layout**: agent col se zvětší pro lepší čitelnost, formulář se zmenší (archiv) |
| `in-development` | `280px 1fr` | **Otočený layout**: zůstává během vývoje |
| `ready-for-pr` | `1fr 280px` | **Vrácený na standard**: vývoj hotov, vrací se na původní proporce |
| `done` | `1fr 280px` | **Vrácený na standard**: story finalizována |
...
.r-main {
  display: grid;
  grid-template-columns: 1fr 280px;  /* normální */
  gap: 12px;
  align-items: start;
}

.r-main.layout-reversed {
  grid-template-columns: 280px 1fr;  /* otočený */
}
```

### Kdy se layout mění

1. **Trigger na reversed**: Po `submitForm()` → session se vytvoří a status → `asking_questions` s `validation_phase: product_owner` → přidej `.layout-reversed`
2. **Trigger zpět na normální**: Když timeline dosáhne kroků `ready-for-pr` (5) nebo `done` (6) → odstraň `.layout-reversed`

### Mapování timeline → layout

| Timeline krok (`key`) | Story status | Layout |
|---|---|---|
| 0: `draft` | `draft` | normální |
| 1: `conflict-check` | `conflict-check` | normální |
| 2: `ready-for-arch` | `ready-for-arch` | normální |
| 3: `arch-approved` | `validated` | **reversed** |
| 4: `in-development` | `in-development` | **reversed** |
| 5: `ready-for-pr` | `ready-for-pr` | normální |
| 6: `done` | `done` | normální |

### Mobile fallback

Na výšku ≤ 700px se `.r-main` už přepíná na `grid-template-columns: 1fr` (existující @media query). Layout toggle zde nemá efekt — zůstává single-column.

<!-- /SECTION: story-ui-layout -->

<!-- SECTION: load-idea-flow -->
## Load Idea Flow — Full Page vs Modal

### Aktuální stav (modal)
- Tlačítko "Načíst myšlenku" v headeru otevírá modal s ID `ghOverlay`
- Modal se zavírá kliknutím mimo nebo na tlačítko X
- Filtrování je client-side (All, idea, story, bug, ready-for-testing)
- Výběr myšlenky vyplní formulář a modal se zavře

### Nový stav (full page)
- Tlačítko "Načíst myšlenku" v headeru přesměruje na `/list`
- `/list` je full-page layout s seznamem myšlenek
- Filtry fungují jako v modálu (single-select)
- Výběr myšlenky vrátí na index.html a vyplní formulář
- Zpět odkaz umožňuje návrat bez výběru

### Header komponenta
- **Lokace**: `.page-head` v index.html, bug.html a list.html
- **Tlačítka**: Načíst myšlenku, Nahlásit bug, Nový požadavek, Vytvořit myšlenku
- **Styling**: jednotný CSS (shodný na všech stránkách)
- **Akcí**:
  - "Načíst myšlenku" → `window.location = '/list'`
  - "Nahlásit bug" → `window.location = '/bug/new'`
  - "Nový požadavek" → `newStory()` (vyčistí formulář, focus na name)
  - "Vytvořit myšlenku" → `createIdea()` (vytvoří idea issue a resetuje)

### Data flow
1. Uživatel klikne "Načíst myšlenku"
2. Přesměrování na `/list`
3. list.js zavolá `/api/issues` → renderuje seznam
4. Filtrování se aplikuje client-side
5. Klik na myšlenku → `selectGhIssue(id)`
6. Data se uloží do localStorage['gh-issue-load'] (nebo query params)
7. Přesměrování na `/`
8. app.js detectuje localStorage a vyplní formulář
9. Timeline se zobrazí po uložení draftu

### Kompatibilita
- Bug issues: Pokud je vybraný issue typu 'bug' → přesměrování na `/bug/new` s prefill daty
- Story/idea: Formulář se vyplní, dialog se neotevírá
